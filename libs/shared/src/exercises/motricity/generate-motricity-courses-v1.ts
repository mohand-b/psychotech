...
import { offsetPolyline } from './motricity-geometry';

export const MOTRICITY_V1_WIDTH_SHRINK = 0.2;
...
function buildCourse(
  ...
  const widths = points
    ...
    .map(
      (_, segmentIndex) =>
        startWidth *
        (1 - MOTRICITY_V1_WIDTH_SHRINK * (segmentIndex / (segmentCount - 1))),
    );
...
export function generateLegacyMotricityCourses(
  seed: string,
